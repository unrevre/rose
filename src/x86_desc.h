...
/* Load task register. This macro takes a 16-bit index into the GDT, which
 * points to the TSS entry. x86 then reads the GDT's TSS descriptor and loads
 * the base address specified in that descriptor into the task register */
#define ltr(desc)                       \
do {                                    \
    asm volatile("ltr   %w0"            \
                 :                      \
...
/* Load the interrupt descriptor table (IDT). This macro takes a 32-bit address
 * which points to a 6-byte structure. The 6-byte structure (defined as "struct
 * x86_desc" above) contains a 2-byte size field specifying the size of the
 * IDT, and a 4-byte address field specifying the base address of the IDT. */
#define lidt(desc)                      \
do {                                    \
    asm volatile("lidt  (%0)"           \
                 :                      \
...
/* Load the local descriptor table (LDT) register. This macro takes a 16-bit
 * index into the GDT, which points to the LDT entry. x86 then reads the GDT's
 * LDT descriptor and loads the base address specified in that descriptor into
 * the LDT register */
#define lldt(desc)                      \
do {                                    \
